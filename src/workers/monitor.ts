...
async function fetchGdeltConflicts(region?: string, country?: string, days: number = 30, limit: number = 100): Promise<Conflict[]> {
  ...
  type GdeltArticle = { title?: string; domain?: string; seendate?: string; sourcecountry?: string; url?: string };
  const data = await res.json() as { articles?: Array<GdeltArticle> };
...
async function fetchAcledConflicts(region?: string, country?: string, days: number = 30, limit: number = 100): Promise<Conflict[]> {
  ...
  type AcledEvent = { country?: string; event_type?: string; region?: string; actor1?: string; actor2?: string; event_date?: string; fatalities?: number; notes?: string };
  const data = await res.json() as { data?: Array<AcledEvent> };
...
async function fetchOpenSkyFlights(
  ...
  type OpenSkyResponse = { states?: OpenSkyState[]; time?: number };
