...
function explodeDemand(
  ...
  for (const comp of components) {
    ...
    // Clamp to 0–99.9% to prevent Infinity from 100% scrap
    const clampedScrap = comp.scrapPercent ? Math.min(comp.scrapPercent, 99.9) : 0;
    const scrapFactor = clampedScrap > 0 ? 1 / (1 - clampedScrap / 100) : 1;
