# a2a-mcp-server Onboarding & Runbook (v3.0.0)
...
Note: `supply-chain` (port 8095) is not included in any profile preset. To enable it, set it explicitly in the `workers` array in config. This worker requires a Business Central or Odoo ERP connection configured at runtime.
...
User-space workers are assigned ports starting at **8096** (since 8095 is used by the built-in supply-chain worker). You can override this in `worker.json`:
```json
{ "name": "my-tool", "port": 8200, "description": "Custom tool" }
```
...
- **Notifications, alert rules, scheduler** → `docs/ARCHITECTURE.md` §13.5–13.7
- **Supply chain, ERP, MRP, ESG modules** → `docs/ARCHITECTURE.md` §20 source tree
...
│   ├── workers/               # 15 built-in worker processes (8081-8095)
│   │   ├── shell.ts           # Port 8081: run_shell, read_file, write_file, SSE stream
│   │   ├── web.ts             # Port 8082: fetch_url, call_api
│   │   ├── ai.ts              # Port 8083: ask_claude, search_files, query_sqlite
│   │   ├── code.ts            # Port 8084: codex_exec, codex_review
│   │   ├── knowledge.ts       # Port 8085: create/read/update/search/list notes
│   │   ├── design.ts          # Port 8086: enhance_ui_prompt, suggest_screens (Gemini)
│   │   ├── factory.ts         # Port 8087: normalize_intent, create_project
│   │   ├── data.ts            # Port 8088: parse_csv, transform_data, analyze_data
│   │   ├── news.ts            # Port 8089: fetch_rss, aggregate_feeds, classify_news
│   │   ├── market.ts          # Port 8090: fetch_quote, price_history, technical_analysis
│   │   ├── signal.ts          # Port 8091: aggregate_signals, classify_threat
│   │   ├── monitor.ts         # Port 8092: track_conflicts, detect_surge, vessel tracking
│   │   ├── infra.ts           # Port 8093: cascade_analysis, chokepoint_assess
│   │   ├── climate.ts         # Port 8094: fetch_earthquakes, fetch_wildfires, assess_exposure
│   │   └── supply-chain.ts    # Port 8095: ERP supply chain, analyze_orders, run_mrp
│   ├── config.ts              # Config loading (Zod-validated)
│   ├── memory.ts              # SQLite + Obsidian dual-write
│   ├── sandbox.ts             # Isolated Bun subprocess executor
│   ├── workflow-engine.ts     # DAG-based multi-step workflows
│   ├── notifications.ts       # Slack/Telegram/Email outbound alerts
│   ├── alert-rules.ts         # Persistent alert rules (SQLite)
│   ├── scheduler.ts           # Recurring skill/workflow scheduler
│   ├── federation.ts          # A2A peer federation
│   ├── worker-registry.ts     # Community worker registry
│   ├── osint-intel.ts         # OSINT workflow builders
│   ├── erp-platform.ts        # ERP platform operations (Q2O, connectors)
